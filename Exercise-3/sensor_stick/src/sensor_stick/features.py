...
def compute_normal_histograms(normal_cloud):
    ...
    norm_1_hist = np.histogram(norm_x_vals, bins=32, range=(0, 256))
    norm_2_hist = np.histogram(norm_y_vals, bins=32, range=(0, 256))
    norm_3_hist = np.histogram(norm_z_vals, bins=32, range=(0, 256))

    # TODO: Concatenate and normalize the histograms
    hist_features = np.concatenate((norm_1_hist[0], norm_2_hist[0], norm_3_hist[0])).astype(np.float64)
    norm_features = hist_features / np.sum(hist_features)
